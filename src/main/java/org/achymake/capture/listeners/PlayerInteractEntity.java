...
public class PlayerInteractEntity implements Listener {
    ...
    public void onPlayerInteractEntity(PlayerInteractEntityEvent event) {
        ...
        if (getMaterialHandler().isPickedUp(mainHand) || getMaterialHandler().isPickedUp(offHand)) {
            event.setCancelled(true);
        } else {
            if (!getMaterialHandler().isCaptureItemStack(mainHand))return;
            if (!getMaterialHandler().isAir(offHand))return;
